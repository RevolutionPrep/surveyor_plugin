    ...
    def retrieve_results(instances, options = {})
      unless options.empty?
        if options[:post] == true
          net_http = Surveyor::ActiveResourceModels::QuestionResult.post(:show, {}, { :api_key => self.survey_key, :results => instances.collect { |i| i.surveyor_result_id }.compact.uniq.join(','), :handles => options[:handles].join(',') }.to_xml)
          typed_result = Surveyor::ActiveResourceModels::QuestionResult.new.load(Surveyor::ActiveResourceModels::QuestionResult.format.decode(net_http.body))
        else
