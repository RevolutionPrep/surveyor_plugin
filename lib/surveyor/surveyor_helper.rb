module SurveyorHelper
  ...
  def display_survey(instances, options = {})
    if instances.to_a.length == 1
      unless instances.to_a.first.surveyor_result_id
        survey = instances.to_a.first.retrieve_survey
      ...
      show_cumulative_survey_results(survey_results,instances.first.class)
    end
  end
  ...
  def show_survey_form(survey,parent_class)
    ...
    parent_class.attributes.each do |attribute|
      unless attribute.last.nil?
        form << "<input id=\"surveyor_#{attribute.first}\" name=\"surveyor[#{attribute.first}]\" type=\"hidden\" value=\"#{attribute.last}\" />"
      end
    end
    ...
    survey.questions.each do |question|
      attribute_name = "#{survey.key}_#{question.id}"
      form << "<li class=\"radio\" id=\"surveyor_#{survey.key}_#{question.id}_input\">"
      form << "<label for=\"surveyor_#{survey.key}_#{question.id}_\" class=\"question#{parent_class.errors.on(attribute_name) ? " is_required" : ""}\">#{question.statement} #{question.required == "true" ? "(required)" : ""}</label>"
      form << "<ul>"
      ...
      when "MultipleChoiceQuestion"
        ...
        question.choices.each do |choice|
          form << "<li><label for=\"surveyor_#{survey.key}_#{question.id}_\"><input id=\"surveyor_#{survey.key}_#{question.id}\" name=\"surveyor[#{survey.key}_#{question.id}][]\" type=\"radio\" value=\"#{choice.id}\" #{parent_class.send(attribute_name).to_s == choice.id.to_s ? 'checked' : nil} /> #{choice.value}</label></li>"
        end
      ...
      when "ShortAnswerQuestion"
        ...
        form << "<textarea cols=\"60\" id=\"surveyor_#{survey.key}_#{question.id}\" name=\"surveyor[#{survey.key}_#{question.id}]\" rows=\"15\">#{parent_class.send(attribute_name)}</textarea>"
        form << "</ul>"
        form << "</li>"
  ...
  def show_cumulative_survey_results(survey_result,klass)
    results = ""
    ...
    survey_result.questions.each do |question|
      ...
      when "MultipleChoiceQuestion", "RatingQuestion"
        ...
        if question.sub_type == "RatingQuestion" && question.handle != 'recommend' && question.handle != 'interested'
          ...
          results << "<tr class=#{cycle("even","odd")}><td/><td>#{question.respondent_count}</td><td>#{'%.2f' % question.gpa_of_respondents rescue nil}</td><td>#{'%.2f' % question.gpa_of_total rescue nil}</td></tr>"
        end
      when "ShortAnswerQuestion"
